...
pub fn update_joypad_state (gameboy: &mut Cpu) {
    ...
    gameboy.mem.joypad.start_pressed = 
      key(Key::Return) || joy(ps4::START);
    
    gameboy.mem.joypad.select_pressed = 
      key(Key::BackSpace) || joy(ps4::TOUCHPAD) || joy(ps4::SHARE);
    
    gameboy.mem.joypad.up_pressed = 
      key(Key::Up) || axis(ps4::LEFT_STICK_Y, false) || axis(ps4::DPAD_Y, true);
    
    gameboy.mem.joypad.down_pressed = 
      key(Key::Down) || axis(ps4::LEFT_STICK_Y, true) || axis(ps4::DPAD_Y, false);
    
    gameboy.mem.joypad.left_pressed = 
      key(Key::Left) || axis(ps4::LEFT_STICK_X, false) || axis(ps4::DPAD_X, false);
    
    gameboy.mem.joypad.right_pressed = 
      key(Key::Right) || axis(ps4::LEFT_STICK_X, true) || axis(ps4::DPAD_X, true);
}
...
fn axis (target: Axis, positive: bool) -> bool {
  for i in 0..joystick::COUNT {
    ...
    if val > 100. - ps4::DEADZONE { return true }
  }
