...
use crate::{combine_u8, cpu::EmulationTarget, memory::ram::Ram};
...
fn palette_spec_write(address: &mut u16, value: u8, auto_increment: &mut bool) {
...
impl PaletteRam {
    ...
    pub fn raw_read(&self, address: u16) -> u8 {
        ...
        match address {
            0xFF68 => {
                palette_spec_read(self.bg_address, self.bg_auto_increment)
            },
            0xFF69 => self.bg_palette_ram.read(self.bg_address),

            0xFF6A => {
                palette_spec_read(self.obj_address, self.obj_auto_increment)
            },
    ...
    pub fn raw_write(&mut self, address: u16, value: u8) {
        ...
        match address {
            0xFF68 => palette_spec_write(
                &mut self.bg_address,
                value,
                &mut self.bg_auto_increment,
            ),
            ...
            0xFF6A => palette_spec_write(
                &mut self.obj_address,
                value,
                &mut self.obj_auto_increment,
            ),
            ...
            _ => panic!(
                "CGB Palette RAM write at {:#06x} (value: {:#04x})",
                address, value
            ),
        }
    }
